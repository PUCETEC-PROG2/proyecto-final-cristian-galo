...
def agregar_producto(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('catalogo_ropa:productos')
    else:   
        form = ProductoForm()
...
def agregar_compra(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            compra.save()
            form.save_m2m() 
            return redirect('catalogo_ropa:detalle_compra', id=compra.id)
    else:
        form = CompraForm()
...
def agregar_cliente(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('catalogo_ropa:index')
    else:   
        form = ClienteForm()
...
def editar_cliente(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('catalogo_ropa:clientes')
    else:
